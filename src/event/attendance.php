...
$meeting_dates_sql =    "SELECT total_attendances.TotalAttendance_Date
                        ...
                        WHERE attendances.Event_ID='$event_id' AND MONTH(total_attendances.TotalAttendance_Date)=$month AND YEAR(total_attendances.TotalAttendance_Date)=$year
...
$prepared_attendance =  $link->prepare("SELECT attendances.Attendance_Presence
                        ...
                        WHERE attendances.User_ID=? AND attendances.Event_ID=? AND total_attendances.TotalAttendance_Date=?");
$prepared_attendance->bind_param("sss", $member_user_id, $event_id, $meeting_date);

if (($event_role == 2 || $event_role == 3) && !isAdmin() && strtotime($event_endday) >= strtotime("now") && $event_approval != "Canceled") { ?>
<!-- Take Attendance -->
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="input-field" style="margin: 0" onchange="takeAttendance(this, '<?php echo $member['User_ID'] ?>', null, '<?php echo $event_id ?>')">
                                <select>
...
<div class="row">
    <div class="col s12">
        ...
        <a class="waves-effect waves-dark btn-flat right" onclick="attendanceReport(null, '<?php echo $event_id ?>')"><i class="material-icons left">print</i>Print Report</a>
        <div id="selected-month" class="input-field right col m3" style="margin: 0; padding: 0" onchange="selectedAttendanceMonth(this, null, '<?php echo $event_id ?>')">
            <select>
